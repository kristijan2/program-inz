...
class PrikazIgre(object):
     ...
     def prikazPocetkaIgre(self):
          print("*" * 50)
          print("*" * 20 + "BRISKULA" + "*" * 20)
          print("*" * 50)
     ...
     def prikaziZog(self, kartaZoga):
          print('Briskula je ' + str(kartaZoga))
          print("*" * 50)

     def izaberiKartuZaBacanje(self, karteZaBacanje):
          text = ">>>Baci kartu\n" + "\n".join(" " + str(i+1) + ") " +
               str(karta) for i, karta in enumerate(karteZaBacanje))
          while True:
               izbor = input(text)
               if izbor.isdigit() and int(izbor) >= 1 and int(izbor)<=len(karteZaBacanje):
                    return int(izbor)-1

     def prikaziBacenuKartu(self, ime, bacenaKarta):
          print(">>>" + ime + " je bacio " + str(bacenaKarta))

     def prikaziPocetakRuke(self, ime):
          print(ime + "baca prvi...")

     def prikaziKrajRuke(self, ime):
          print(ime + "je dobio ruku...")
          print("*" * 50)
